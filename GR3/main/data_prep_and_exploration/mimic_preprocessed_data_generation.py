...
mimic_data_dir = '/path/to/your/mimic-iv-clinical-database-2.2/'

# Read extracted time series data.
events = pd.read_csv('mimic_iv_events_text.csv', usecols=['HADM_ID', 'ICUSTAY_ID', 'CHARTTIME', 'VALUENUM', 'TABLE', 'NAME'])
icu = pd.read_csv('mimic_iv_icu_text.csv')
...
# Save data.
pickle.dump([events, oc, train_ind, valid_ind, test_ind], open('mimic_iv_preprocessed_text.pkl','wb'))

# Normalize data and save. 
ts, oc, train_ind, valid_ind, test_ind = pickle.load(open('mimic_iv_preprocessed_text.pkl','rb'))
...
print(oc)
pickle.dump([ts, oc, train_ind, valid_ind, test_ind], open('mimic_iv_preprocessed_text.pkl','wb'))